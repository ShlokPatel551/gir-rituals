...
function targetKey(phone, email, purpose) {
  return `${purpose}:${phone}:${email.toLowerCase()}`;
}
function sendOtp(phone, email, purpose) {
  const now = Date.now();
  ...
  if (existing && now - existing.lastSentAt < RESEND_COOLDOWN_MS) {
    const wait = Math.ceil((RESEND_COOLDOWN_MS - (now - existing.lastSentAt)) / 1e3);
  ...
  return {
    ok: true,
    expiresInSeconds: OTP_EXPIRY_MS / 1e3,
    demoCode: code
  };
}
function verifyOtp(phone, email, purpose, code) {
  const now = Date.now();
  const key = targetKey(phone, email, purpose);
  const all = loadOtps();
...
function getResendCooldownSeconds(phone, email, purpose) {
  const key = targetKey(phone, email, purpose);
  ...
  return Math.ceil((RESEND_COOLDOWN_MS - elapsed) / 1e3);
}
const OTP_EXPIRY_MINUTES = OTP_EXPIRY_MS / 6e4;
export {
  OTP_EXPIRY_MINUTES,
  generateOtpCode,
  getActiveOtps,
