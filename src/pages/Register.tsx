...
const STEPS: { num: Step; label: string; icon: string }[] = [
  { num: 1, label: 'Personal',  icon: '👤' },
  { num: 2, label: 'Billing',   icon: '🏠' },
  ...
  function validate(): boolean {
    ...
    if (step === 1) {
      ...
      if (emailExists(form.email)) {
        setError('This email is already registered. Please log in.'); return false;
      }
      ...
      if (form.password.length < 8 || !/\d/.test(form.password) || !/[^A-Za-z0-9]/.test(form.password)) {
        setError('Password needs 8+ characters, a number, and a special character.'); return false;
      }
    ...
    if (step === 2) {
      if (!form.street.trim() || !form.city.trim() || !form.state.trim() || !form.pinCode.trim()) {
        setError('All billing address fields are required.'); return false;
      }
      if (!/^\d{6}$/.test(form.pinCode)) {
        setError('PIN code must be 6 digits.'); return false;
      }
    }
    if (step === 3 && !form.sameAsBilling) {
      if (!form.deliveryStreet.trim() || !form.deliveryCity.trim() || !form.deliveryState.trim() || !form.deliveryPin.trim()) {
        setError('All delivery address fields are required.'); return false;
      }
      if (!/^\d{6}$/.test(form.deliveryPin)) {
        setError('Delivery PIN code must be 6 digits.'); return false;
      }
    }
    return true;
  }
    ...
    savePendingRegistration({
      user: {
        ...
        deliveryAddress: form.sameAsBilling
          ...
          : { street: form.deliveryStreet, city: form.deliveryCity, state: form.deliveryState, pinCode: form.deliveryPin },
      },
      ...
      <div className="auth-brand-panel">
        <div className="auth-brand-content">
          ...
          <div className="auth-brand-values">
            {[
              { icon: '🌅', label: 'Morning Deliveries' },
              { icon: '🥛', label: 'A2 Certified' },
            ...
            {STEPS.map((s, i) => {
              ...
              return (
                ...
                >
                  ...
                  {i < STEPS.length - 1 && (
                    <div className={`reg-step-line ${done ? 'reg-step-line-done' : ''}`} />
                  )}
                </div>
              );
            })}
          ...
          <form onSubmit={handleSubmit} className="auth-form">
            ...
            {step === 3 && (
              <>
                ...
                {form.sameAsBilling ? (
                  <div className="reg-same-address-preview">
                    <span>📍</span>
                    <div>
                      <p>{form.street}</p>
