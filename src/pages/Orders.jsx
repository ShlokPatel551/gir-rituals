...
function Orders() {
  ...
  const [cancelled, setCancelled] = useState(/* @__PURE__ */ new Set());
  ...
  const handleCancel = (id, _name) => {
    if (!window.confirm(`Cancel order ${id}?`)) return;
    setCancelled((prev) => /* @__PURE__ */ new Set([...prev, id]));
    showToast(`Order ${id} cancelled.`, "info");
  };
  const handleReorder = (productId, name) => {
  ...
  return <div>
      ...
      {list.length === 0 && <div className="empty-state card">
    ...
    return <div key={o.id} className="card" style={{ marginBottom: "0.75rem" }}>
    ...
    >
              ...
              {(tab === "history" || isCancelled) && <button
      type="button"
...
function OrderDetail() {
  ...
  return <div>
      ...
      <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
        ...
        {order.status === "completed" && <button type="button" className="btn btn-primary" style={{ flex: 1 }} onClick={handleReorder}>
            🔄 Re-order
          </button>}
      </div>
    </div>;
