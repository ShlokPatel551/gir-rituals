  ...
  milk: [
    {
      ...
      steps: [
        ...
        "Add \xBD tsp turmeric + pinch of black pepper",
    ...
    },
    {
      ...
      steps: [
        ...
        "Cool to 40\xB0C, stir in 2 tbsp fresh curd as starter",
...
function ProductDetail() {
  ...
  >
          {favourites.includes(product.id) ? "\u2764\uFE0F Favourited" : "\u{1F90D} Add to Favourites"}
        </button>
      </div>
    ...
    /* Best Recipes accordion */
      ...
      {recipes.length > 0 && <div className="card" style={{ marginBottom: "1rem" }}>
          <h3 style={{ marginBottom: "0.75rem" }}>🍽 Best Recipes</h3>
          {recipes.map((recipe, i) => <div key={i} style={{ borderTop: i > 0 ? "1px solid var(--border)" : "none" }}>
              <button
    type="button"
  ...
  >
              ...
              {openRecipe === i && <ol style={{ paddingLeft: "1.25rem", marginBottom: "0.75rem", lineHeight: 1.75 }}>
                  {recipe.steps.map((step, j) => <li key={j} style={{ fontSize: "0.9rem", color: "var(--text-muted)", marginBottom: "0.25rem" }}>
                      {step}
                    </li>)}
                </ol>}
            </div>)}
  ...
  >
    ...
    /* Subscribe modal */
      ...
      {subscribeModal && <div className="modal-overlay" onClick={() => setSubscribeModal(false)}>
