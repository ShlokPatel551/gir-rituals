...
const TYPE_FILTER_OPTIONS = ["All Types", "Discount Codes", "Seasonal Deals", "Loyalty"];
const STATUS_FILTER_OPTIONS = ["Active Status", "Scheduled", "Expired"];
function AdminOffers() {
  const navigate = useNavigate();
  const [typeFilter, setTypeFilter] = useState("All Types");
  const [statusFilter, setStatusFilter] = useState("Active Status");
  const visible = PROMOS.filter((p) => {
    const typeOk = typeFilter === "All Types" || typeFilter === "Discount Codes" && p.type === "discount" || typeFilter === "Seasonal Deals" && p.type === "seasonal" || typeFilter === "Loyalty" && p.type === "loyalty";
    ...
    /* ── Loyalty rewards ── */
      ...
      <div className="bento-card promo-loyalty-card">
        ...
        <div className="promo-loyalty-perks">
          {LOYALTY_PERKS.map((perk) => <div key={perk.icon} className="promo-perk-card">
  ...
  >
        ...
        <div className="admin-table-wrap" style={{ border: "none", borderRadius: 0 }}>
          <table className="admin-table promo-table">
            ...
            <tbody>
              {visible.map((p) => <tr key={p.id} className={p.expired ? "promo-row-expired" : ""}>
                  <td>
  ...
  />
                  ...
                  <td>
                    {p.status === "active" ? <span className="promo-status promo-status-active">
                        <span className="promo-status-dot promo-dot-active" />
                        Active
                      </span> : p.status === "expired" ? <span className="promo-status promo-status-expired">
                        <span className="promo-status-dot promo-dot-expired" />
                        Expired
                      </span> : <span className="promo-status promo-status-scheduled">
        ...
        <div className="promo-table-footer">
          ...
          <div className="promo-pag">
            ...
            {[1, 2, 3].map((n) => <button key={n} type="button" className={`promo-pag-num ${n === 1 ? "promo-pag-active" : ""}`}>{n}</button>)}
    ...
    /* Usage trend chart */
        ...
        <div className="bento-card promo-chart-card">
          ...
          <div className="promo-bars-area">
            {USAGE_BARS.map((b) => <div key={b.label} className="promo-bar-col">
                <div className="promo-bar" style={{ height: `${b.pct}%` }} />
              </div>)}
          </div>
          <div className="promo-bars-labels">
            {USAGE_BARS.map((b) => <span key={b.label}>{b.label}</span>)}
          </div>
        </div>
