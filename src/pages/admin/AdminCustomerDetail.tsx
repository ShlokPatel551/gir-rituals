...
/* ── Mock detail data keyed by clientId ── */
...
const MOCK_MAP: Record<string, CustomerDetail> = {
...
type DeliveryStatus = 'today' | 'delivered' | 'extra' | 'paused' | 'cancelled';

interface DeliveryRow {
  date: string;
  dayNum: number; // day of current month for calendar dot
  product: string;
  qty: string;
  rate: string;
  amount: string;
...
const MOCK_HISTORY: DeliveryRow[] = [
...
type TabKey = 'schedule' | 'bills' | 'orders' | 'transactions' | 'notes';

/* ── Component ── */
export function AdminCustomerDetail() {
  const { id } = useParams<{ id: string }>();

  // Try real customers first, then mock map
  const realCustomers = getAllCustomers();
  const realCustomer = realCustomers.find((c) => c.clientId === id);
  const mockCustomer = id ? MOCK_MAP[id] : undefined;

  const customer: CustomerDetail | undefined = realCustomer
    ? {
  ...
  // Map delivery dots to day numbers
  const dotMap: Record<number, DeliveryStatus> = {};
  ...
  return (
    <div className="cd-page">
      ...
      {/* ── Bills tab ── */}
      {activeTab === 'bills' && (
        <div className="bento-card cd-tab-panel">
      ...
      {activeTab === 'notes' && (
        <div className="cd-notes-section">
          ...
          <div className="cd-notes-list">
            {notes.map((note, i) => (
              <div key={i} className="bento-card cd-note-card">
      ...
      {activeTab === 'schedule' && (
        <div className="bento-card cd-inline-notes">
