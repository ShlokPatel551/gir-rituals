...
function AdminDeliveries() {
  ...
  function toggleAll() {
    setSelected(allChecked ? /* @__PURE__ */ new Set() : new Set(deliveries.map((d) => d.id)));
  }
  ...
  function markDelivered(id) {
    setDeliveries(
      (prev) => prev.map((d) => d.id === id ? { ...d, status: "delivered" } : d)
    );
    setSelected((prev) => {
      const next = new Set(prev);
      next.delete(id);
  ...
  function markAllDelivered() {
    setDeliveries((prev) => prev.map((d) => ({ ...d, status: "delivered" })));
    setSelected(/* @__PURE__ */ new Set());
  }
  function markSelectedFailed() {
    setDeliveries(
      (prev) => prev.map((d) => selected.has(d.id) ? { ...d, status: "paused" } : d)
    );
    setSelected(/* @__PURE__ */ new Set());
  }
  const counts = {
  ...
  >{row.name}</span>
                  ...
                  <td style={{ textAlign: "right" }}>
                    {row.status === "delivered" ? <button type="button" className="adm-view-btn" disabled>Delivered</button> : row.status === "paused" ? <button type="button" className="adm-view-btn" onClick={(e) => {
    e.stopPropagation();
    navigate(`/admin/customers/${row.clientId}`);
