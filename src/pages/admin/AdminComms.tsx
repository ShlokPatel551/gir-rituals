...
type Channel     = 'all' | 'whatsapp' | 'sms' | 'email';
type ConvChannel = 'whatsapp' | 'sms' | 'email';
type MsgDir      = 'in' | 'out';
type MsgStatus   = 'read' | 'delivered';
...
const CONVERSATIONS: Conversation[] = [
  {
    id: 'c1', name: 'Meera Sharma', initials: 'MS',
    avatarBg: '#c1ecd4', avatarColor: '#012d1d',
    plan: 'A2 Desi Ghee Plan',
    preview: '"Is the morning delivery on track?"',
    channel: 'whatsapp', time: '2m ago', online: true,
    lastStatus: 'read', previewItalic: true,
  },
  {
    id: 'c2', name: 'Rohan Verma', initials: 'RV',
    avatarBg: '#ffdcc4', avatarColor: '#2f1400',
    plan: 'Bulk Order Enquiry',
    preview: '"Sent the quotation for the upcoming event..."',
    channel: 'email', time: '1h ago', online: false,
    lastStatus: 'delivered', previewItalic: false,
  },
  {
    id: 'c3', name: 'Priya Kapur', initials: 'PK',
    avatarBg: '#ffdcc4', avatarColor: '#5f2f00',
    plan: 'Trial Period',
    preview: '"The Paneer quality was exceptional, thank you!"',
    channel: 'sms', time: 'Yesterday', online: false,
    lastStatus: 'read', previewItalic: false,
  },
];
...
const QUICK_TEMPLATES = [
  ...
  { label: 'Product Launch 🥛',     text: 'Exciting news! We have a new product available just for you.'            },
...
const CH_BADGE: Record<ConvChannel, { label: string; bg: string; color: string }> = {
...
const CH_LABEL: Record<ConvChannel, string> = {
  whatsapp: 'WhatsApp',
  email:    'Email',
  sms:      'SMS',
};
...
export function AdminComms() {
  ...
  const visibleConvs = channel === 'all'
    ? CONVERSATIONS
  ...
  function handleDraft(val: string) {
    setDraft(val);
    resizeTextarea();
  }
  ...
  function handleSend() {
    ...
    const msg: ChatMsg = { id: `m${Date.now()}`, dir: 'out', text: draft.trim(), time, status: 'delivered' };
          ...
          <div className="comms-filters-body">
            ...
            <div className="comms-section">
              ...
              <div className="comms-seg-list">
                {SEGMENTS.map((seg) => (
                  <label key={seg} className="comms-seg-label">
                    <input
                      type="checkbox"
                  ...
                  <div className="comms-conv-footer">
                    ...
                    <div className={`comms-read-row ${conv.lastStatus === 'read' ? 'comms-read-green' : 'comms-read-muted'}`}>
                      <span
                        ...
                        style={{
                          fontSize: 14,
                          fontVariationSettings: conv.lastStatus === 'read' ? "'FILL' 1" : undefined,
                        }}
                      >
        ...
        <div className="comms-chatwin">
          ...
          <div className="comms-messages">
            ...
            {activeMessages.map((msg) => (
              ...
              >
                {msg.dir === 'in' && (
                  <div className="comms-bubble-avatar" style={{ background: activeConv.avatarBg, color: activeConv.avatarColor }}>
                    {activeConv.initials}
                  </div>
                )}
            ...
            <div className="comms-templates">
              {QUICK_TEMPLATES.map((t) => (
                <button
                  ...
                  onClick={() => { setDraft(t.text); setTimeout(resizeTextarea, 0); }}
                >
                  {t.label}
                </button>
              ))}
