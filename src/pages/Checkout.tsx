...
export function Checkout() {
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '1rem' }}>
      ...
      {step === 3 && (
        <div className="card">
          {['upi', 'netbanking', 'card', 'paymonthly'].map((m) => (
            <label key={m} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 0', cursor: 'pointer' }}>
              ...
              {m === 'paymonthly' && '📅 Pay Monthly'}
            </label>
          ))}
          {walletBalance > 0 && (
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem', background: 'var(--green-50)', borderRadius: 8, marginTop: '0.75rem', cursor: 'pointer' }}>
      ...
      {step === 4 && (
        <div className="card">
          ...
          <div style={{ marginTop: '0.75rem', paddingTop: '0.75rem', borderTop: '1px solid var(--border)', fontSize: '0.9rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--text-muted)' }}><span>GST (5%)</span><span>₹{gst.toFixed(2)}</span></div>
            {walletApplied > 0 && (
              <div style={{ display: 'flex', justifyContent: 'space-between', color: 'var(--green-700)' }}><span>Wallet credit</span><span>− ₹{walletApplied.toFixed(2)}</span></div>
            )}
          </div>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
        ...
        {step < 4 ? (
          <button type="button" className="btn btn-primary" style={{ flex: 1 }} onClick={() => setStep(step + 1)}>Continue</button>
        ) : (
          <button type="button" className="btn btn-primary" style={{ flex: 1 }} onClick={handleConfirm}>Confirm & Pay</button>
        )}
      </div>
    </div>
  );
