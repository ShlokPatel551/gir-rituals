...
function AccountSettings() {
  ...
  return <div>
      ...
      {successMessage && <p className="card" style={{ marginBottom: "1rem", fontSize: "0.9rem", color: "var(--md-primary)" }}>
          {successMessage}
        </p>}
      ...
      <div className="card">
        ...
        {paymentMethods.map((pm) => <div key={pm.id} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.5rem 0", borderBottom: "1px solid var(--border)" }}>
            <span style={{ flex: 1 }}>
              {pm.type === "upi" ? "\u{1F4F2}" : pm.type === "card" ? "\u{1F4B3}" : "\u{1F3E6}"} {pm.label}
            </span>
            {pm.isDefault && <span className="badge badge-delivered" style={{ fontSize: "0.7rem" }}>Default</span>}
            {!pm.isDefault && <button type="button" style={{ fontSize: "0.75rem", background: "none", color: "var(--green-700)", border: "none", cursor: "pointer" }} onClick={() => setDefaultPaymentMethod(pm.id)}>
                Set Default
              </button>}
        ...
        {showAddMethod && <div style={{ marginTop: "1rem", padding: "0.75rem", background: "var(--green-50)", borderRadius: 8 }}>
  ...
  return <div>
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", gap: "0.25rem", marginBottom: "1rem" }}>
          {[1, 2, 3, 4, 5].map((n) => <button key={n} type="button" onClick={() => setRating(n)} style={{ background: "none", fontSize: "1.5rem" }}>{n <= rating ? "\u2605" : "\u2606"}</button>)}
        </div>
