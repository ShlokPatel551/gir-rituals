...
function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const { phone } = await api.forgotPassword(email.trim());
      sessionStorage.setItem("gir_reset_email", email.trim());
      sessionStorage.setItem("gir_reset_password", password);
      navigate(`/otp?purpose=reset_password&phone=${encodeURIComponent(phone)}&email=${encodeURIComponent(email.trim())}`);
    } catch (err) {
