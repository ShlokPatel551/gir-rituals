...
export function Schedule() {
  ...
  const deliveredDays = Array.from({ length: daysInMonth }, (_, i) => i + 1)
    .filter((d) => getDayStatus(d, viewMonth, viewYear, today) === 'delivered').length;
  const pausedDays = Array.from({ length: daysInMonth }, (_, i) => i + 1)
    .filter((d) => getDayStatus(d, viewMonth, viewYear, today) === 'paused').length;
  const extraOrders = Array.from({ length: daysInMonth }, (_, i) => i + 1)
    .filter((d) => getDayStatus(d, viewMonth, viewYear, today) === 'extra').length;
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: '1rem' }}>
        ...
        <div className="calendar-grid">
          {Array.from({ length: daysInMonth }, (_, i) => i + 1).map((day) => {
            ...
            return (
              ...
              >
                {day}
                {dotClass && <span className={dotClass}>{status === 'extra' ? dotLabel : ''}</span>}
              </button>
            );
          })}
        </div>
        ...
        <div style={{ marginTop: '0.5rem', fontSize: '0.75rem', color: 'var(--text-muted)', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          ...
          <span><span style={{ color: '#9ca3af' }}>●</span> No Order</span>
        </div>
      </div>

      <Link to={`/schedule/day/${selectedDay}`} className="card" style={{ display: 'block', marginBottom: '1rem', color: 'var(--green-700)', fontWeight: 600 }}>
        View Day Detail — {selectedDay} {getMonthLabel(viewMonth, viewYear)} →
      </Link>
      ...
      {entries.length === 0 && (
        <div className="empty-state card" style={{ marginBottom: '1rem' }}>
          <p>No delivery scheduled for {selectedDay} {getMonthLabel(viewMonth, viewYear)}</p>
        </div>
      )}
      ...
      <div className="card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', marginBottom: '0.75rem' }}>
          <div className="card" style={{ background: 'var(--green-50)', padding: '0.75rem' }}>
      ...
      {entries.length === 0 ? (
        ...
        entries.map((e, i) => (
          <div key={i} className="card" style={{ marginBottom: '0.5rem' }}>
            ...
            <p style={{ margin: '0.25rem 0' }}>Qty: {e.qty} {e.qty === 1 ? 'litre' : 'litres'} · Rate: ₹{e.rate}/litre · Total: ₹{e.qty * e.rate}</p>
