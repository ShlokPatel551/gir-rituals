...
const QUICK_LINKS = [
  { to: "/schedule", icon: "\u{1F4C5}", label: "My Schedule" },
  { to: "/bills", icon: "\u{1F9FE}", label: "My Bills" },
  { to: "/orders", icon: "\u{1F4CB}", label: "Orders" },
  { to: "/products", icon: "\u{1F4E6}", label: "Products" },
  { to: "/offers", icon: "\u{1F381}", label: "Offers" },
  { to: "/profile", icon: "\u{1F464}", label: "Profile" }
];
const statementTypeIcon = {
  delivery: "\u{1F4E6}",
  extra: "\u2795",
  pause: "\u23F8",
  payment: "\u2705",
...
function Dashboard() {
  ...
  return <div className="dashboard-page">
      ...
      <div className="dashboard-banner">
        <div>
          ...
          <p className="dashboard-sub">
            {unpaidCount > 0 ? `You have ${unpaidCount} unpaid bill${unpaidCount > 1 ? "s" : ""} \u2014 ` : "Everything is up to date \xB7 "}
            {unpaidCount > 0 && <Link to="/bills" style={{ color: "var(--md-primary-container)", fontWeight: 600 }}>
                Pay now
              </Link>}
        ...
        <div className="stat-card">
          ...
          <div>
            <p className="stat-value">
              {(/* @__PURE__ */ new Date()).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}
            </p>
    ...
    /* Daily Rituals */
          ...
          <section className="dashboard-section">
            ...
            {rituals.length === 0 ? <div className="card empty-state">
    ...
    return <div key={r.id} className="ritual-row card">
                    ...
                    <span className={`badge badge-${r.status === "Delivered" ? "delivered" : r.status === "Paused" ? "paused" : r.status === "Extra" ? "extra" : r.status === "Out for Delivery" ? "out-for-delivery" : r.status === "Cancelled" ? "cancelled" : "pending"}`}>
                      {r.status}
                    </span>
                  </div>;
  })}
  ...
  >
        ...
        <aside className="dashboard-sidebar">
          <div className="dashboard-section">
            ...
            <div className="quick-links-grid">
              {QUICK_LINKS.map((l) => <Link key={l.to} to={l.to} className="quick-link-card card">
    ...
    /* Unpaid bill alert */
          ...
          {unpaidCount > 0 && <div className="unpaid-alert card">
    ...
    /* Notifications widget */
          ...
          <div className="dash-notif-card">
            <div className="dash-notif-header">
              ...
              {notifications.filter((n) => !n.read).length > 0 && <span className="dash-notif-badge">
                  {notifications.filter((n) => !n.read).length}
                </span>}
            </div>
            <div className="dash-notif-list">
              {notifications.slice(0, 4).map((n) => <Link
    key={n.id}
