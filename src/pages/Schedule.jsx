...
function buildScheduleForDay(day, month, year, today) {
  ...
  const deliveryStatus = status === "paused" ? "Paused" : status === "extra" ? "Extra" : status === "cancelled" ? "Cancelled" : "Delivered";
  return [
    {
...
function Schedule() {
  ...
  const filteredDays = filterStatus === "all" ? allDays : allDays.filter((d) => getDayStatus(d, viewMonth, viewYear, today) === filterStatus);
  return <div>
      {
    /* Header row */
  }
  ...
  >
    ...
    /* Day detail */
          ...
          <div className="card" style={{ marginBottom: "0.75rem" }}>
            ...
            {entries.length === 0 ? <p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>No delivery scheduled for this day.</p> : <table style={{ width: "100%", fontSize: "0.85rem", borderCollapse: "collapse" }}>
                <thead>
                ...
                <tbody>
                  {entries.map((e, i) => <tr key={i}>
                      ...
                      <td>₹{e.qty * e.rate}</td>
                      <td>
                        <span className={`badge badge-${e.status === "Delivered" ? "delivered" : e.status === "Paused" ? "paused" : e.status === "Extra" ? "extra" : "unpaid"}`}>{e.status}</span>
                      </td>
                    </tr>)}
                </tbody>
              </table>}
...
function DayDetail() {
  ...
  return <div>
      ...
      {entries.length === 0 ? <div className="empty-state card"><p>No deliveries scheduled for this day.</p></div> : entries.map((e, i) => <div key={i} className="card" style={{ marginBottom: "0.5rem" }}>
      ...
      {entries.length > 0 && <Link to={`/payment?amount=${entries.reduce((s, e) => s + e.qty * e.rate, 0)}&from=schedule`} className="btn btn-primary" style={{ marginTop: "1rem", display: "block", textAlign: "center" }}>
          Pay for this day
        </Link>}
    </div>;
}
