...
export function Otp() {
  ...
  const purpose = (searchParams.get('purpose') as OtpPurpose) || 'register';
  ...
  }, []); // eslint-disable-line react-hooks/exhaustive-deps -- send once on mount
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    if (purpose === 'register') {
      ...
      const user: User = { ...reg.user, clientId };
  ...
  return (
    <div className="auth-page otp-page">
      ...
      <p className="auth-footer">
        {resendSec > 0 ? (
          <>Resend OTP in <strong>{resendSec}s</strong></>
        ) : (
