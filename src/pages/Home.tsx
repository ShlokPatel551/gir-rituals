...
const BRAND_VALUES = [
  { icon: '🐄', title: 'A2 Gir Milk', desc: 'Pure A2 protein from indigenous Gir cows' },
  { icon: '🌿', title: 'No Preservatives', desc: 'Farm-fresh, additive-free, every day' },
  { icon: '🚚', title: 'Daily Delivery', desc: 'At your door by 7 AM, rain or shine' },
  { icon: '🫙', title: 'Bilona Ghee', desc: 'Hand-churned using ancient Vedic methods' },
];
...
const WHY_ITEMS = [
  { icon: '🏡', title: 'From Our Farm', desc: 'Our Gir cows roam freely on 50+ acres of natural pastures in Gir, Gujarat.' },
  { icon: '🧪', title: 'Quality Tested', desc: 'Every batch is tested for adulteration and quality before reaching your home.' },
    ...
    <div className="home-page">
      ...
      <section className="home-section">
        <div className="brand-values-strip">
          {BRAND_VALUES.map((v) => (
            <div key={v.title} className="brand-value-tile">
              ...
              <div key={ritual.id} className="ritual-card card">
                <div className="ritual-header">
                  ...
                  <span className={`badge badge-${
                    ...
                    ritual.status === 'Cancelled' ? 'cancelled' :
      ...
      {activeOffers.length > 0 && (
        <section className="home-section">
          ...
          {activeOffers.map((o) => {
            ...
            return (
              <Link key={o.id} to={`/offers/${o.id}`} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem', textDecoration: 'none' }}>
                <div>
                  ...
                  {o.promoCode && (
                    <span style={{ fontFamily: 'monospace', fontSize: '0.8rem', fontWeight: 700, color: 'var(--green-700)', background: 'var(--md-primary-container)', padding: '0.15rem 0.4rem', borderRadius: 4, marginTop: '0.25rem', display: 'inline-block' }}>
                      {o.promoCode}
                    </span>
                  )}
                </div>
